...
<xsl:template name="hyphenate.verbatim">
  ...
  <xsl:template name="RSAPILegalNotice">
        <xsl:if test="@role = 'rs-api'">
            <fo:block xsl:use-attribute-sets="normal.para.spacing">
                <xsl:value-of select="/*/d:info/d:abstract"/> The
                document is for informational purposes only and is
                provided “AS IS.” </fo:block>
            <fo:block xsl:use-attribute-sets="normal.para.spacing">
                RACKSPACE MAKES NO REPRESENTATIONS OR WARRANTIES OF
                ...
                FOR A PARTICULAR PURPOSE, AND NONINFRINGEMENT. </fo:block>
            <fo:block xsl:use-attribute-sets="normal.para.spacing">
